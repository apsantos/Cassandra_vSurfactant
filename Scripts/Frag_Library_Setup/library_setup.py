...
for line_number,line in enumerate(in_file):
    ...
    if line_parse[0]=='#':        
        ...
        elif line_parse[1] == "Rcutoff_Low":
            rcut_line = line_number+1
...
#Obtain Rcut
rcut_low = linecache.getline(input_file, rcut_line+1)
...
#Obtain VDW_Style info
vdw_style = []
for i in xrange(0,nbr_boxes):
    vdw_style.append(linecache.getline(input_file, vdw_style_line+1+i))

#Obtain Intra_Scaling info
intra_scaling = []
for i in xrange(0,nbr_species):
    intra_scaling.append(linecache.getline(input_file, intra_scaling_line+1+(2*i)) +
                         linecache.getline(input_file, intra_scaling_line+2+(2*i)) )

#Obtain box sizes info
box_sizes = []
for i in xrange(0,nbr_boxes):
    box_sizes.append(linecache.getline(input_file, box_info_line+2+(2*i)) +
                     linecache.getline(input_file, box_info_line+3+(2*i)) )
print 'box', box_sizes
